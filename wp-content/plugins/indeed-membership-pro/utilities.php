...
Bank NAME<br/>',
		...
		case 'payment_payza':
			$arr = array(
					'ihc_payza_status' => 0,
					'ihc_payza_email' => '',
					'ihc_payza_label' => 'Payza',
					'ihc_payza_select_order' => 7,
			);
			break;
		case 'login':
			$arr = array(
...
<br/>',
		...
		case 'account_page':
			$arr = array(	'ihc_ap_theme' => 'ihc-ap-theme-3',
							...
							'ihc_ap_welcome_msg' => '<span class="iump-user-page-mess-special">Pozdrav</span> <span class="iump-user-page-mess-special"> {last_name} {first_name}</span>,
														<span class="iump-user-page-mess">ulogirani ste kao</span><span class="iump-user-page-mess-special"> {username}</span>
														<div class="iump-user-page-mess"><span>{flag}</span>Član od {user_registered}</div>
														',
							'ihc_account_page_custom_css' => '',
							'ihc_ap_social_plus_message' => '',

							'ihc_ap_overview_menu_label' => 'Moje ideje',
							'ihc_ap_overview_title' => 'Moji ideje',
							...
							'ihc_ap_profile_menu_label' => 'Moj Profil',
							'ihc_ap_profile_title' => 'Moj Profil',
							'ihc_ap_profile_msg' => '',
							...
							'ihc_ap_user_sites_label' => 'User Sites',
							'ihc_ap_user_sites_title' => 'User Sites',
...
<div class="iump-clear"></div>',
		...
		case 'ihc_membership_card':
			$arr = array(
							...
							'ihc_membership_member_since_label' => __('Član od: ', 'ihc'),
...
function ihc_native_user_field(){
	...
	$arr = array(
			...
			array( 'display_admin'=>0, 'display_public_reg'=>0, 'display_public_ap'=>0, 'name'=>'ihc_country', 'label'=>'Country', 'type'=>'ihc_country', 'native_wp' => 0, 'req' => 2, 'sublevel' => '' ),
			...
			array( 'display_admin'=>0, 'display_public_reg'=>1, 'display_public_ap'=>0, 'name'=>'ihc_dynamic_price', 'label'=>'Price', 'type'=>'ihc_dynamic_price', 'native_wp' => 0, 'req' => 0, 'sublevel' => '' ),
	);
	...
	if (isset($attr['type']) && $attr['type']){
		switch ($attr['type']){
			...
			case 'conditional_text':
				...
				$str = '<input type="text" name="'.$attr['name'].'" '. $id_field .' class="'.$attr['class'].'" value="' . ihc_correct_text($attr['value']) . '" placeholder="'.$attr['placeholder'].'" '.$attr['other_args'].' '.$attr['disabled'].' />';
			...
			case 'textarea':
				$id_field = (isset($attr['id']) && $attr['id'] != "" ) ? 'id="'.$attr['id'].'"' : '';
				$str = '<textarea name="'.$attr['name'].'" '. $id_field .' class="iump-form-textarea '.$attr['class'].'" '.$attr['other_args'].' '.$attr['disabled'].' >' . ihc_correct_text($attr['value']) . '</textarea>';
				if (!empty($attr['sublabel'])){
					$str .= '<label class="iump-form-sublabel">' . ihc_correct_text($attr['sublabel']) . '</label>';
				}
				break;
			...
			case 'checkbox':
				...
				if ($attr['multiple_values']){
					...
					foreach ($attr['multiple_values'] as $v){
						...
						$str .= '<input type="checkbox" name="'.$attr['name'].'[]" '. $id_field .' class="'.$attr['class'].'" value="' . ihc_correct_text($v) . '" '.$checked.' '.$attr['other_args'].' '.$attr['disabled'].'  />';
			...
			case 'radio':
				...
				if ($attr['multiple_values']){
					...
					foreach ($attr['multiple_values'] as $v){
						...
						$str .= '<input type="radio" name="'.$attr['name'].'" '. $id_field .' class="'.$attr['class'].'" value="' . ihc_correct_text($v) . '" '.$checked.' '.$attr['other_args'].' '.$attr['disabled'].'  />';
						$str .= ihc_correct_text($v);
						$str .= '</div>';
					}
					$str .= '</div>';
			...
			case 'submit':
				$id_field = (isset($attr['id']) && $attr['id'] != "" ) ? 'id="'.$attr['id'].'"' : '';
				$str = '<input type="submit" value="' . ihc_correct_text($attr['value']) . '" name="'.$attr['name'].'" '. $id_field .' class="'.$attr['class'].'" '.$attr['other_args'].' '.$attr['disabled'].' />';
				if (!empty($attr['sublabel'])){
					$str .= '<label class="iump-form-sublabel">' . ihc_correct_text($attr['sublabel']) . '</label>';
				}
				break;
			...
			case 'upload_image':
								...
								jQuery("#ihc_fileuploader_wrapp_' . $rand . ' .ihc-file-upload").uploadFile({
									...
									onSuccess: function(a, response, b, c){
										if (response){
											...
											jQuery("#ihc_fileuploader_wrapp_' . $rand . ' .ihc-file-upload").prepend("<div onClick=\"ihc_delete_file_via_ajax("+obj.id+", ' . $attr['user_id'] . ', \'#ihc_fileuploader_wrapp_' . $rand . '\', \'' . $attr['name'] . '\', \'#ihc_upload_hidden_'.$rand.'\');\" class=\'ihc-delete-attachment-bttn\'>Remove</div>");
											jQuery("#ihc_fileuploader_wrapp_' . $rand . ' .ihc-file-upload").prepend("<img src="+obj.url+" class=\'ihc-member-photo\' /><div class=\'ihc-clear\'></div>");
				...
				if ($attr['value']){
					...
					if (isset($url)){
						...
						$str .= '<input type="hidden" value="" name="ihc_avatar"  id="ihc_upload_hidden_'.$rand.'" />';
					}
				} else {
			...
			case 'unique_value_text':
				...
				$str = '<input type="text" data-search-unique="true" onBlur="ihc_check_unique_value_field(\'' . $attr['name'] . '\');" name="'.$attr['name'].'" id="'.$attr['id'].'" class="'.$attr['class'].'" value="' . ihc_correct_text($attr['value']) . '" placeholder="'.$attr['placeholder'].'" '.$attr['other_args'].' '.$attr['disabled'].' />';
				if (!empty($attr['sublabel'])){
					$str .= '<label class="iump-form-sublabel">' . ihc_correct_text($attr['sublabel']) . '</label>';
				}
				break;
			...
			case 'ihc_country':
				...
				$onchange = 'onChange="ihc_update_state_field();';
			...
			case 'ihc_dynamic_price':
				if (strcmp($attr['form_type'], 'create')===0 && ihc_is_magic_feat_active('level_dynamic_price') && !empty($attr['lid'])){
					...
					if (!empty($temp_settings['ihc_level_dynamic_price_levels_on'][$lid])){
						...
						$str .= "<input type='number' onChange='ihc_dynamic_price_update_global();' onBlur='ihc_dynamic_price_update_global();' min='$min' max='$max' class='{$attr['class']}' step='$step' value='$level_price' name='ihc_dynamic_price' id='ihc_dynamic_price' />";
					}
				}
				break;
		}
...
function ihc_print_bank_transfer_order($u_id, $l_id){
	...
	if ($reccurrence){
		if (isset($level_data['access_trial_price']) && $level_data['access_trial_price']!==''){
			$amount = $level_data['access_trial_price'];
		}
...
function ihc_handle_levels_assign($uid=0, $lid=-1, $custom_start_time=0, $custom_end_time=0){
	/*
	 * Used in : buy new level ( with Authorize recurring or Stripe), social login
	 * insert into db when user was start using this level,
	 * @param int (user id)
	 * @param int (level id)
	 * @param string (start time - optional)
	 * @param string (end time - optional)
	 * @return bool, true if succeed
	 */
	if ($uid && $lid!=-1){
		...
		if (!empty($custom_start_time)){
			$start_time = date('Y-m-d H:i:s', strtotime($custom_start_time));
		} else {
			$start_time = date('Y-m-d H:i:s', $start_time);
		}
		if (!empty($custom_end_time)){
			$end_time = date('Y-m-d H:i:s', strtotime($custom_end_time));
		}
		...
		foreach ($users as $user){
				foreach ($register_fields as $v){
						if (isset($user->data->{$v['name']})){
								$the_user_data[] = $user->data->{$v['name']};
						} else {
				...
				$levels = Ihc_Db::get_user_levels($user->data->ID);
				if ($levels){
						/// with levels
...
function ihc_cancel_level($u_id, $l_id, $dont_redirect_paypal=FALSE){
	 ...
	 * @param u_id - user id (int), l_id - level id (int), boolean
	 * @return none
	 */
	$txn_id = '';
	$payment_type = '';
...
function ihc_delete_coupon($id){
	 ...
	 */
	global $wpdb;
	$q = $wpdb->prepare("SELECT * FROM ".$wpdb->prefix."ihc_coupons WHERE id=%d;", $id);
	$exists = $wpdb->get_row($q);
	if ($exists){
...
function ihc_meta_value_exists($meta_key='', $meta_value=''){
	...
	if ($meta_key && $meta_value){
		...
		$q = $wpdb->prepare("SELECT * FROM $table WHERE meta_value=%s AND meta_key=%s ;", $meta_value, $meta_key);
		$data = $wpdb->get_results($q);
		if (!empty($data)){
			return TRUE;
		}
	 ...
	 if ($type){
	 	switch ($type){
			...
			case 'login_level_redirect':
				...
				break;
			case 'register_redirects_by_level':
				return get_option('ihc_register_redirects_by_level_enable');
				break;
			case 'wp_social_login':
			...
			case 'badges':
				...
				break;
			case 'login_security':
				return get_option('ihc_login_security_on');
				break;
			case 'workflow_restrictions':
				return get_option('ihc_workflow_restrictions_on');
				break;
			case 'subscription_delay':
				return get_option('ihc_subscription_delay_on');
				break;
			case 'level_dynamic_price':
				return get_option('ihc_level_dynamic_price_on');
				break;
			case 'user_reports':
				return get_option('ihc_user_reports_enabled');
				break;
			case 'pushover':
				return get_option('ihc_pushover_enabled');
				break;
			case 'account_page_menu':
				return get_option('ihc_account_page_menu_enabled');
				break;
			case 'mycred':
				return get_option('ihc_mycred_enabled');
				break;
			case 'api':
				return get_option('ihc_api_enabled');
				break;
			case 'woo_product_custom_prices':
				return get_option('ihc_woo_product_custom_prices_enabled');
				break;
			case 'drip_content_notifications':
				return get_option('ihc_drip_content_notifications_enabled');
				break;
			case 'user_sites':
				return get_option('ihc_user_sites_enabled');
				break;
	 	}
	 }
	 return FALSE;
